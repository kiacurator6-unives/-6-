...
//
// 1. SHEET MODE (API URL configured): entries are read from and written to
//    a Google Sheet via a Google Apps Script Web App, so everyone who opens
//    this page sees everyone else's entries. See GOOGLE_SHEETS_SETUP.md.
//
// 2. LOCAL MODE (no API URL yet): falls back to localStorage, which is
//    per-browser only — entries stay on the device that typed them and are
//    not visible to other people. A "복사하기" button lets people copy their
//    entry to paste elsewhere until Sheet mode is set up.
...
(function () {
  ...
  function saveLocal(section, entries) {
    try {
      ...
      /* private browsing / storage disabled — entry still renders this session */
    }
  }
  ...
  function renderCard(entry, index, section) {
    ...
    const deleteBtn = SHEET_MODE
      ? ''
      : `<button type="button" class="danger remove">삭제</button>`;
    return `
  ...
  function renderModeNote() {
    ...
    if (!noteEl) return;
    if (SHEET_MODE) {
      noteEl.innerHTML = `<div><b>안내</b>모든 사람에게 실시간으로 공유되는 구글 시트에 연결되어 있어요. 자유롭게 소개를 남겨주세요.</div>`;
    } else {
  ...
  function setupForm(section) {
    ...
    form.addEventListener('submit', async (ev) => {
      ...
      const entry = { name, role, message };

      setFormBusy(form, true);
      try {
        if (SHEET_MODE) {
          await postToSheet(section, entry);
          cache[section].push(entry);
        } else {
