...
(function () {
  ...
  // Single source of truth: true only when config is filled in AND the client
  // actually initialized. Prevents the UI from claiming "connected" while
  // silently running in local-only mode (e.g. if the CDN script failed to load).
  const ACTIVE = SUPABASE_MODE && !!sb;
  ...
  function renderCard(entry, index, section) {
    ...
    const deleteBtn = ACTIVE ? '' : `<button type="button" class="danger remove">삭제</button>`;
    return `
  ...
  function setupListActions(section) {
    ...
    listEl.addEventListener('click', (ev) => {
      ...
      if (btn.classList.contains('copy')) {
        ...
        copyText(text, btn);
      } else if (btn.classList.contains('remove') && !ACTIVE) {
        const entries = loadLocal(section);
